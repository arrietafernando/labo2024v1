...
AgregaVarRandomForest <- function(
    rf_num.trees, rf_max.depth, rf_min.node.size, rf_mtry, rf_semilla, 
    rf_train_from = 202101, rf_train_to = 202101) {
  ...
  set.seed(rf_semilla, kind = "L'Ecuyer-CMRG")
  azar <- runif(nrow(dataset_rf))

  dataset_rf[, entrenamiento :=
               as.integer(foto_mes >= rf_train_from & foto_mes <= rf_train_to &
                            (clase01 == 1 | azar < 0.10))]
...
CanaritosAsesinos <- function(
    ...
    canaritos_train_from = 202101, canaritos_train_to = 202101, 
    canaritos_valid = 202103) {
  ...
  dataset[, entrenamiento := 
            foto_mes >= canaritos_train_from & foto_mes <= canaritos_train_to & 
            (clase01 == 1 | azar < 0.10)]
  ...
  dvalid <- lgb.Dataset(
    data = data.matrix(dataset[foto_mes == canaritos_valid, campos_buenos, with = FALSE]),
    label = dataset[foto_mes == canaritos_valid, clase01],
    weight = dataset[
      foto_mes == canaritos_valid,
      ifelse(clase_ternaria == "BAJA+2", 1.0000001, 1.0)
    ],
    free_raw_data = FALSE
  )
...
BorutaFilter <- function( boruta_semilla, boruta_max_run, 
                          boruta_train_from, boruta_train_to ) {
  ...
  boruta_out <- Boruta(clase01~., 
                       data=dataset_boruta[entrenamiento==TRUE, ..campos_buenos], 
                       doTrace=2, 
                       maxRuns=boruta_max_run)
...
if (PARAM$RandomForest$run) {
  ...
  AgregaVarRandomForest(
    rf_num.trees = PARAM$RandomForest$num.trees,
    rf_max.depth = PARAM$RandomForest$max.depth,
    rf_min.node.size = PARAM$RandomForest$min.node.size,
    rf_mtry = PARAM$RandomForest$mtry,
    rf_semilla = PARAM$RandomForest$semilla,
    rf_train_from = PARAM$RandomForest$train_from,
    rf_train_to = PARAM$RandomForest$train_to
  )
...
if (PARAM$CanaritosAsesinos$ratio > 0.0) {
  ...
  CanaritosAsesinos(
    ...
    canaritos_train_from = PARAM$CanaritosAsesinos$train_from,
    canaritos_train_to = PARAM$CanaritosAsesinos$train_to,
    canaritos_valid = PARAM$CanaritosAsesinos$valid
  )
